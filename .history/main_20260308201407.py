...
def draw_explosions_and_particles():
    for exp in explosions:
        ...
        if timer > max_t * 0.6:
            ...
            screen.blit(txt, (int(x) - 52, int(y) - 44))
...
def draw_bottom_dashboard(stats, missile_list):
    """Clearly visible bottom status bar."""
    dash_y = 800
    dash_h = 58
...
def draw_left_panel():
    ...
    for i, (k, v, c) in enumerate(lines):
        y = py + 30 + i * 22
        screen.blit(font_small.render(f"  {k:<12}: ", True,
                    config.DIM_GREEN), (px, y))
    ...
    while True:
        ...
        if spawn_timer >= config.MISSILE_SPAWN_RATE:
            for _ in range(config.MISSILE_SPAWN_COUNT):
                m = Missile()
                # override spawn to use new center
                import math as _math, random as _random
                angle  = _random.uniform(0, 2 * _math.pi)
                cx, cy = RADAR_CENTER
                m.x    = cx + RADAR_RADIUS * _math.cos(angle)
                m.y    = cy + RADAR_RADIUS * _math.sin(angle)
                tx     = cx + _random.uniform(-30, 30)
                ty     = cy + _random.uniform(-30, 30)
                dx     = tx - m.x
                dy     = ty - m.y
        ...
        for m in missiles:
            m.update()
            # check against new center
            import math as _math
            cx, cy = RADAR_CENTER
            if _math.hypot(m.x - cx, m.y - cy) <= config.PROTECTED_RADIUS:
                m.alive = False
