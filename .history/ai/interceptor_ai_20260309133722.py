...
class InterceptorAI:
    ...
    def _coordinate_swarm(self):
        """
        If multiple interceptors chase the same target,
        assign one as flanker with lateral offset.
        """
        # group interceptors by target id
        from collections import defaultdict
        target_groups = defaultdict(list)
        for inc in self.interceptors:
            if inc.alive and inc.target \
                    and inc.target.alive:
                target_groups[inc.target.id].append(inc)

        for target_id, group in target_groups.items():
            if len(group) < 2:
                # single interceptor — no flanking needed
                group[0].is_flanking  = False
                group[0].flank_offset = 0.0
                continue

            # sort by distance to target
            group.sort(key=lambda i: math.hypot(
                i.x - i.target.x,
                i.y - i.target.y))

            # closest one goes direct
            group[0].is_flanking  = False
            group[0].flank_offset = 0.0

            # others flank alternating left/right
            for k, inc in enumerate(group[1:], 1):
                sign = 1 if k % 2 == 0 else -1
                inc.is_flanking  = True
                inc.flank_offset = sign * \
                    config.SWARM_FLANK_OFFSET        
