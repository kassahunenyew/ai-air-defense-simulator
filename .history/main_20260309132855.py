...
import random
import config
from simulation.missile import Missile, TYPE_BALLISTIC
...
from visualization.dashboard import AIDashboard
...
def spawn_explosion(x, y):
    ...
    for _ in range(40):
        ...
        particles.append([
            ...
            random.randint(40,70), color, "debris"
        ])
    # fire particles
    for _ in range(20):
        angle = random.uniform(-math.pi*0.4,
                                math.pi*0.4)
        speed = random.uniform(0.5, 3.5)
        size  = random.randint(2, 5)   # was 3, 8
        particles.append([
            x, y,
            math.cos(angle)*speed*random.uniform(0.5,1.5),
            math.sin(angle)*speed - random.uniform(1,3),
            random.randint(50, 90),
            (255, random.randint(40,160), 0),
            "fire",
            size
        ])
    print(f"spawned explosion: {len([p for p in particles if len(p)>6 and p[6]=='fire'])} fire particles")
    ...
    for _ in range(25):
        ...
        particles.append([
            x, y,
            math.cos(angle)*speed,
            math.sin(angle)*speed - random.uniform(0.5,2),
...
def draw_crt_effect(explosion_intensity=0.0):
    # ── Scanlines ─────────────────────────────────
    scanline_surf = pygame.Surface((1400, 860),
                                   pygame.SRCALPHA)
    for y in range(0, 860, 2):
        pygame.draw.line(scanline_surf, (0, 0, 0, 60),
                         (0, y), (1400, y), 1)
    screen.blit(scanline_surf, (0, 0))

    # ── Vignette (dark edges) ─────────────────────
    vignette_surf = pygame.Surface((1400, 860),
                                   pygame.SRCALPHA)
    cx, cy = 700, 430
    for r, alpha in [(500, 0), (480, 10),
                     (450, 25), (400, 50),
                     (350, 80), (300, 110)]:
        pygame.draw.circle(vignette_surf,
                           (0, 0, 0, alpha),
                           (cx, cy), r,
                           60)
    screen.blit(vignette_surf, (0, 0))

    # ── Phosphor bloom on explosion ───────────────
    if explosion_intensity > 0.01:
        bloom_surf = pygame.Surface((1400, 860),
                                    pygame.SRCALPHA)
        alpha = int(60 * explosion_intensity)
        bloom_surf.fill((0, min(alpha, 40), 0, 
                         min(alpha, 50)))
        screen.blit(bloom_surf, (0, 0))

    # ── Subtle green phosphor overlay ─────────────
    phosphor = pygame.Surface((1400, 860),
                               pygame.SRCALPHA)
    phosphor.fill((0, 20, 5, 8))
...
def draw_explosions_and_particles():
    ...
    for p in particles:
        ptype = p[6] if len(p) > 6 else "debris"

        if ptype == "fire":
            size   = max(1, int(p[7] * p[4]/90))  # was max(2,...)
            alpha  = max(0, min(255, int(220 * p[4]/90)))
            r,g,b  = p[5]
            surf = pygame.Surface((size*2+1, size*2+1),
                                  pygame.SRCALPHA)
            # outer glow — smaller
            ...
            size  = max(1, int(p[7] * p[4]/120))
            alpha = int(100 * p[4]/120)
            r,g,b = p[5]
...
def draw_ew_jamming(missile_list):
    """Amber jamming radius ring around active jammers."""
    for m in missile_list:
        if not m.alive:
            continue
        if not getattr(m, 'is_jamming', False):
            continue
        jf = m.jamming_factor
        if jf < 0.05:
            continue

        # pulsing jamming ring
        pulse = abs(math.sin(frame_count * 0.15)) * 0.5 \
                + 0.5
        alpha = int(120 * jf * pulse)
        r     = int(config.EW_JAMMING_RANGE)

        surf = pygame.Surface((1400, 860),
                               pygame.SRCALPHA)
        pygame.draw.circle(
            surf,
            (255, 200, 0, alpha // 3),
        ...
        # jamming label
        lbl = font_small.render(
            f"⚡ JAMMING {int(jf*100)}%",
            True, config.EW_JAMMING_COLOR)
        screen.blit(lbl, (int(m.x) + 20,
                          int(m.y) - 24))

        # radar static effect near jammer
        if jf > 0.3:
            import random
            for _ in range(int(20 * jf)):
                sx = int(m.x) + random.randint(
                    -int(r*0.8), int(r*0.8))
                sy = int(m.y) + random.randint(
                    -int(r*0.8), int(r*0.8))
        ...
        for inc in interceptor_ai.interceptors:
            if inc.hit:
                spawn_explosion(inc.target.x,
                                inc.target.y)
                bloom_intensity = 1.0
                dashboard.log_intercept(      # ← add this
                    inc.target.x, inc.target.y)
