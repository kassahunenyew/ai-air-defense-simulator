...
class Interceptor:
    def __init__(self, target):
        ...
        self.hit      = False
        self.trail    = []
        self.id       = target.id    # tracks which missile it chases
    ...
    def update(self):
        ...
        # -- steer toward target every frame (proportional navigation) --
        self._update_velocity()
        ...
        # -- check intercept (hit if within 10 pixels) --
        dist = math.hypot(self.x - self.target.x,
                          self.y - self.target.y)
        if dist < 10:
            self.hit         = True
            self.alive       = False
            self.target.alive = False    # destroy the missile
