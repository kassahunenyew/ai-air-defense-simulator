# main.py
# ============================================================
# ENTRY POINT — LSTM + RL + Hungarian + RCS Detection
# ============================================================
...
def draw_missile_shape(surface, x, y, vx, vy, color,
    ...
    canard_top = [rot(body_len*0.35, -body_w),
                  rot(body_len*0.20, -body_w*4.0),
                  rot(body_len*0.10, -body_w)]
    canard_bot = [rot(body_len*0.35,  body_w),
                  rot(body_len*0.20,  body_w*4.0),
                  rot(body_len*0.10,  body_w)]
    ...
    tail_top = [rot(-body_len*0.30, -body_w),
                rot(-body_len*0.30 - size*1.1, -body_w*4.5),
                rot(-body_len*0.05, -body_w)]
    tail_bot = [rot(-body_len*0.30,  body_w),
                rot(-body_len*0.30 - size*1.1,  body_w*4.5),
                rot(-body_len*0.05,  body_w)]
    ...
    flame_layers = [(size*0.9,  (255,220,80)),
                    (size*0.6,  (255,120,20)),
                    (size*0.35, (255,60, 0))]
...
def draw_missiles(ranked_list):
    for i, (m, score, tti) in enumerate(ranked_list):
        ...
        for j, (tx, ty) in enumerate(m.trail):
            ...
            trail_bright = max(0.2, prob)
            tc           = (int(min(alpha,255) * trail_bright),
                            0, 0)
...
def draw_legend():
    ...
    screen.blit(font_small.render(
        "RCS BALLISTIC: 1.0m²",
        True, config.DIM_GREEN), (px, py3+22))
    screen.blit(font_small.render(
        "RCS EVASIVE:   0.3m²",
        True, config.DIM_GREEN), (px, py3+36))
    screen.blit(font_small.render(
        "RCS STEALTH:  0.01m²",
        True, (100,200,100)), (px, py3+50))
...
def draw_hud_frame():
    ...
    screen.blit(font_small.render(
        "KALMAN  |  LSTM  |  RL AGENT  |  "
        "HUNGARIAN  |  RCS DETECTION",
