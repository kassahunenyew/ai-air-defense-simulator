...
def draw_targeting_bracket(x, y, size, color, frame,
                           is_locked=False):
    """
    Animated corner brackets that close in on lock.
    Rotates slightly when locked.
    """
    x, y   = int(x), int(y)
    rotation = (frame * 0.5) % 360 if is_locked else 0
    s      = size
    gap    = int(s * 0.35)
    thick  = 2 if not is_locked else 2

    # pulsing size when locked
    if is_locked:
        pulse = math.sin(frame * 0.15) * 3
        s     = int(s + pulse)

    corners = [
        (-s, -s, 1, 0, 0, 1),   # top-left:    right, down
        ( s, -s,-1, 0, 0, 1),   # top-right:   left, down
        (-s,  s, 1, 0, 0,-1),   # bottom-left: right, up
        ( s,  s,-1, 0, 0,-1),   # bottom-right:left, up
    ]
    ...
    arm = int(s * 0.45)
    for cx2, cy2, dx1, dy1, dx2, dy2 in corners:
        p0 = rrot(cx2, cy2)
        p1 = rrot(cx2 + dx1*arm, cy2 + dy1*arm)
        p2 = rrot(cx2 + dx2*arm, cy2 + dy2*arm)
        pygame.draw.line(screen, color, p0, p1, thick)
        pygame.draw.line(screen, color, p0, p2, thick)

    # center crosshair on locked target
    if is_locked:
        cross = 6
        pygame.draw.line(screen, color,
                         (x-cross, y), (x+cross, y), 1)
        pygame.draw.line(screen, color,
                         (x, y-cross), (x, y+cross), 1)
...
def draw_engagement_lines(interceptors):
    ...
    for inc in interceptors:
        ...
        lbl  = font_small.render(f"{int(dist)}px",
                                  True, (0,120,200))
        screen.blit(lbl, (mx+4, my-8))
...
def draw_missiles(ranked_list, frame):
    ...
    for i, (m, score, tti) in enumerate(ranked_list):
        ...
        draw_targeting_bracket(m.x, m.y, bracket_size,
...
def draw_interceptors(inc_list, frame):
    ...
    for inc in inc_list:
        ...
        draw_targeting_bracket(inc.x, inc.y, 18,
...
def draw_legend():
    ...
    screen.blit(font_small.render(
        "[ ] TARGETING BRACKET",True,(255,255,100)),(px,py4+22))
    screen.blit(font_small.render(
