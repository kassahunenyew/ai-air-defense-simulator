# main.py
# ============================================================
# ENTRY POINT — FINAL VERSION WITH LSTM + RL + MISSILE TYPES
# ============================================================
...
def draw_missiles(ranked_list):
    for i, (m, score, tti) in enumerate(ranked_list):
        ...
        label = font_small.render(f"TGT-{m.id} [{t}]", True, color)
...
def draw_lstm_paths(missile_list):
    """Cyan LSTM predicted paths."""
    for m in missile_list:
        if not m.alive:
            continue
        if not m.lstm_predicted_path:
            continue
        if not m.is_visible_on_radar():
            continue
...
def draw_legend():
    ...
    for i, (label, color) in enumerate(types):
...
def draw_hud_frame():
    ...
    screen.blit(font_small.render(
        "KALMAN TRACKING  |  LSTM PREDICTION  "
        "|  THREAT AI  |  AUTO INTERCEPT",
    ...
    while True:
        ...
        if spawn_timer >= config.MISSILE_SPAWN_RATE:
            for _ in range(config.MISSILE_SPAWN_COUNT):
                ...
                # each type has realistic targeting behavior
                if m.type == "BALLISTIC":
                    # precision strike on protected zone
                    tx = cx + random.uniform(-20, 20)
                    ty = cy + random.uniform(-20, 20)

                elif m.type == "EVASIVE":
                    # hits random inner infrastructure
                    inner_angle = random.uniform(0, 2 * math.pi)
                    inner_r     = random.uniform(
                                      0, RADAR_RADIUS * 0.4)
                    tx = cx + inner_r * math.cos(inner_angle)
                    ty = cy + inner_r * math.sin(inner_angle)

                elif m.type == "STEALTH":
                    # sneaky mid-radar approach
                    inner_angle = random.uniform(0, 2 * math.pi)
                    inner_r     = random.uniform(
                                      RADAR_RADIUS * 0.1,
                                      RADAR_RADIUS * 0.5)
        ...
        # -- notify evasive missiles of nearby interceptors --
        for m in missiles:
            if m.type == "EVASIVE":
        ...
        # -- detect hits → spawn explosions + sound --
        for inc in interceptor_ai.interceptors:
            if inc.hit:
